...
set VERSION=v0.5.0-2k25-beta3
set EXENAME=tgc_gui_2k25_beta3
set PKG=TGC-Designer-Tools-2K25-v0.5.0-2k25-beta3-Windows-x64
set RELEASEBASE=dist\release_beta3
set RELEASEDIR=%RELEASEBASE%\%PKG%

echo.
echo ============================================================
echo TGC DESIGNER TOOLS 2K25 - BETA 3 BUILD
echo ============================================================
echo.
...
echo.
echo === Beta 3 source identity check ===
python -c "from pathlib import Path; s=Path('tgc_gui.py').read_text(encoding='utf-8'); req=['TGC_GUI_VERSION = \"v0.5.0-2k25-beta3\"','TGC_APP_TITLE = \"TGC Designer Tools 2K25 - Beta 3\"','root.title(TGC_APP_TITLE)']; miss=[x for x in req if x not in s]; print('Source title: TGC Designer Tools 2K25 - Beta 3' if not miss else 'ERROR: this folder still contains an older tgc_gui.py'); raise SystemExit(1 if miss else 0)"
if errorlevel 1 (
  echo.
  echo ERROR: You are NOT building from the Beta 3 source tree in this folder.
  echo Expected:
  echo   TGC_GUI_VERSION = v0.5.0-2k25-beta3
  echo   Window title = TGC Designer Tools 2K25 - Beta 3
  echo.
  echo Make sure the Beta 3 package files were extracted DIRECTLY into:
...
echo.
echo === Clean previous Beta 3 build ===
...
copy /y "RELEASE_NOTES_v0.5.0-2k25-beta3.md" "%RELEASEDIR%\" >nul
